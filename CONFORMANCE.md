...
validation rule, paired with the diagnostic a conforming validator must produce. **60 of the
...
| Lexer-level grammar (§11.2–11.4) | `brace-stray`, `value-curly-quote`, `raw-html`, `anchor-misplaced` |
...
Cases for the rules above are skipped by name, so the 35 `not implemented` skips reproduce this
table one for one. The run reports 42 skips in total: the remaining seven are the implemented
